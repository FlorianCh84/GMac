...
final class OpenAIProvider: LLMProvider, Sendable {
    ...
    init(keychain: any KeychainServiceProtocol = KeychainService(), model: String = LLMProviderType.openai.defaultModel) {
    ...
    private func complete(conversation: LLMConversation) async throws -> String {
        ...
        if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
            ...
            throw LLMError.requestFailed("OpenAI \(http.statusCode): \(msg)")
        }
